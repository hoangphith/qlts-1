<?php

namespace app\modules\dungchung\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\modules\dungchung\models\TaiLieu;

/**
 * TaiLieuSearch represents the model behind the search form about `app\modules\dungchung\models\TaiLieu`.
 */
class TaiLieuSearch extends TaiLieu
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['id', 'id_tham_chieu', 'nguoi_tao'], 'integer'],
            [['loai', 'ten_tai_lieu', 'duong_dan', 'ten_file_luu', 'file_extension', 'ghi_chu', 'thoi_gian_tao'], 'safe'],
            [['file_size'], 'number'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params, $cusomSearch=NULL)
    {
        $query = TaiLieu::find();

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }
		if($cusomSearch != NULL){
			$query->andFilterWhere ( [ 'OR' ,['like', 'loai', $cusomSearch],
            ['like', 'ten_tai_lieu', $cusomSearch],
            ['like', 'duong_dan', $cusomSearch],
            ['like', 'ten_file_luu', $cusomSearch],
            ['like', 'file_extension', $cusomSearch],
            ['like', 'ghi_chu', $cusomSearch]] );
 
		} else {
        	$query->andFilterWhere([
            'id' => $this->id,
            'id_tham_chieu' => $this->id_tham_chieu,
            'file_size' => $this->file_size,
            'thoi_gian_tao' => $this->thoi_gian_tao,
            'nguoi_tao' => $this->nguoi_tao,
        ]);

        $query->andFilterWhere(['like', 'loai', $this->loai])
            ->andFilterWhere(['like', 'ten_tai_lieu', $this->ten_tai_lieu])
            ->andFilterWhere(['like', 'duong_dan', $this->duong_dan])
            ->andFilterWhere(['like', 'ten_file_luu', $this->ten_file_luu])
            ->andFilterWhere(['like', 'file_extension', $this->file_extension])
            ->andFilterWhere(['like', 'ghi_chu', $this->ghi_chu]);
		}
        return $dataProvider;
    }
}
